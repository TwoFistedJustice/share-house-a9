...
const state = {
  ...
  *  There is not just one setter                                 */
  activeHouse:
    { //pull it from db based houseId - found in userModule.nameFull
      houseName: '',
      members: []
      // member looks like [ {id: userId, nameFull: firstLast, idAdmin: t/f, role: ''} ]
    }
...
const actions = {
      ...
      globalAxios.patch('/houses/' + houseId + '.json/?auth=' + token, formData)
        .then(resp => {
          ...
          if (resp.data.houseName === formData.houseName) {
            alert('house nameFull changed successfully to ' + resp.data.houseName);
          }
          return resp;
        })
        .then(function () {
    ...
    fetchActiveHouse({commit, dispatch}) {
      ...
      * get the  user id from local storage - don't pass it as an arg!!!!
        ...
        use the memberIds to get the member's names
            - use for loop to set member nameFull in house obj* */
        ...
        globalAxios.get('/users/' + memberArray[i].id + '.json?auth=' + token)
          .then(response => {
            ...
            let memberBlob = {
              ...
              nameFirst: response.data.name.first,
              nameLast: response.data.name.last,
    ...
    instantiateHouse({commit, dispatch}, pushId) {
      ...
      /* this is the unique key generated by firebase for the house node nameFull */
